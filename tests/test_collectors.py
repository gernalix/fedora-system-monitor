...
"""
    ...
    def test_running_oneshot_uses_previous_completed_success_for_freshness(self) -> None:
        unit = "amici-fb.service"
        self.db.set_state("services.runtime", {unit: {"last_success_exit_monotonic_us": 900_000_000}}, namespace="collector")
...
"""
        ...
        with (
            ...
            mock.patch.object(periodic.time, "monotonic", return_value=1000.0),
        ):
            result = periodic.collect_services("minute", config, self.db)
        metric = next(row for row in result.metrics if row.get("device_id") == unit and row["name"] == "service.active")
        self.assertTrue(metric["details"]["running_oneshot"])

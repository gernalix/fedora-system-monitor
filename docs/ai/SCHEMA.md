...
TABLE.dedup_state=Collector deltas,alert state,cooldown,journal cursors,event coalescing,and persistent Telegram free-space baselines
STATE.telegram_namespace=notification
STATE.telegram_key=filesystem-free:<UUID_or_stable_fallback>
STATE.telegram_value=notified_free_bytes,last_observed_free_bytes,current_mount_point
STATE.telegram_retention=Notification namespace is not age-pruned;state survives service and host restarts and unmounted intervals
...
DERIVED.context_index=Git-friendly correlated view; canonical sources remain monitor.sqlite3 and activity-watch-data
DERIVED.context_window_schema=fedora-system-monitor.context-window.v1
DERIVED.incident_bundle_schema=fedora-system-monitor.incident-bundle.v1
DERIVED.context_layout=metadata/,timeline/YYYY/MM/YYYY-MM-DD.jsonl,incidents/<incident-id>.json,summaries/YYYY-MM-DD.json
DERIVED.context_identity=Stable record_id from monitor row id or ActivityWatch content hash; overlapping sync replaces its time slice
DERIVED.context_privacy=Raw ActivityWatch URLs are excluded; strings pass through monitor redaction
